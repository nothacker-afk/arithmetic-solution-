...
def test_socketio_instance_configured():
    """Sanity: socketio object exists and has the expected event handlers."""
    assert socketio is not None
    # Handlers are registered at import time
    handlers = socketio.handlers.get("/", {})
    for event in ("join", "leave", "disconnect", "broadcast_calc", "ping_presence"):
        assert event in handlers, f"Missing socket handler: {event}"
